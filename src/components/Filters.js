...
const Filters = () => {
  ...
  const componies=getUniqueValues(all_products,'company')
  const colors=getUniqueValues(all_products,'colors')
  console.log(colors)
 
  return <Wrapper>Фильтр
  ...
  <div className="colors">
    {
      colors.map((c,index)=>{
        ...
        return <button key={index} name="color" style={{background:c}} onClick={updateFilters} className={`${color===c? 'color-btn active':'color-btn'}`} data-color={c}>{
...
<div className="form-control">
  ...
  <input value={price} type="range" name='price' onChange={updateFilters} min={min_price} max={max_price} />

</div>
<div className="form-control shipping">
  <label htmlFor="shipping">Арзан <i class="fas fa-shipping-fast    "></i></label>
<input type="checkbox" name='shipping' id='shipping' onChange={updateFilters} checked={shipping} />
</div>
      </form>
      <button type='button' className='clear-btn' onClick={clearFilters}>Тазалау</button>
    </div>
  </Wrapper>
}
